...
export class ManageUsers {
    ...
    constructor(http: Http, router: Router) {
        ...
        http.get('/admin/users/list').map((res) => res.json()).subscribe((res) => {
            ...
            for (let i in res.data) this.users.push(res.data[i]);
        })
    }
    editUser(userId) { this.router.navigate(['/Settings', 'EditUser', { userId: userId }]); }
    deleteUser(userId) {
        ...
        this.http.post('admin/users/' + userId + '/delete', '').map((res) => res.json())
